...
export class QuickSortComponent implements OnInit {
   ...
   */
  static partition(array: number[], start: number, before: number): void {
    ...
    QuickSortComponent.partition(array, start, pivotRank);

    /** Partition all the elements more than the pivot */
    QuickSortComponent.partition(array, pivotRank + 1, before);
  }
  ...
  static quickSort(array: number[]): number[] {
    array = array.slice();
    QuickSortComponent.partition(array, 0, array.length);
    return array;
  }
  ...
  ngOnInit() {
    console.log(QuickSortComponent.quickSort(this.unsortedArray), ': QuickSortComponent');
  }
